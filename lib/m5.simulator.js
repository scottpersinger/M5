...
M5.onTouchReady(function() {
  ...
  function remote_send(message) {
    //socket.send(message);
    $.post('send_command__/' + remote_key, {command: message})
  }
  ...
  function poll_responses() {
    if (!remote_key)
      return;
    $.get('get_response__/' + remote_key, function(response) {
      if (response != "") {
  ...
  function remote_connect() {
    $.post('remote_connect__/' + M5.settings.app_name, function(result) {
      remote_key = result.key;
    }, 'json');
    
    cmd_interval = setInterval(poll_commands, 200);
    res_interval = setInterval(poll_responses, 250);
  ...
  if (M5.settings.simulator_console) {
    remote_connect();
  }
